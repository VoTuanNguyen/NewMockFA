...
public interface BookingRepository extends JpaRepository<Booking, Integer> {
	...
	@Query("SELECT b FROM Booking b WHERE b.user.id = :user_id ORDER BY b.id DESC")
	Page<Booking> getListBookingByUser(@Param("user_id") int user_id, Pageable pageable);

	@Query("SELECT b FROM Booking b WHERE b.user.id = :user_id AND (b.date > :date OR (b.date = :date AND CAST(b.trip.time as int) > :time)) AND b.status = 1 ORDER BY b.id DESC")
	Page<Booking> getListBookingByUserFilterWaiting(@Param("user_id") int user_id, @Param("date") Date date,
			@Param("time") int time, Pageable pageable);

	@Query("SELECT b FROM Booking b WHERE b.user.id = :user_id AND (b.date < :date OR (b.date = :date AND CAST(b.trip.time as int) < :time)) ORDER BY b.id DESC")
	Page<Booking> getListBookingByUserFilterExpired(@Param("user_id") int user_id, @Param("date") Date date,
			@Param("time") int time, Pageable pageable);

	@Query("SELECT b FROM Booking b WHERE b.user.id = :user_id AND b.status = 0 AND (b.date > :date OR (b.date = :date AND CAST(b.trip.time as int) > :time)) ORDER BY b.id DESC")
